...
ssh -p ${SSH_PORT} ${SSH_USER}@${SSH_HOST} \
    ...
    sudo mv app_upstream_${[TYPE]_IMAGE_TAG} app_upstream_${[TYPE]_IMAGE_TAG}.old && \
    sleep 10 && \
    echo 'proxy_pass http://app-${[TYPE]_IMAGE_TAG}-${BITBUCKET_BUILD_NUMBER};' | sudo tee app_upstream_${[TYPE]_IMAGE_TAG} && \
